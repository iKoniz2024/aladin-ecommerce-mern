                ...
                <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Define attributes for products under this category</p>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={() => createAttrAppend({ key: "", label: "", type: "text", options: "", required: false, unit: "" })}
                    >
                      <Plus className="size-3 mr-1" /> Add Attribute
                    </Button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {createAttrFields.map((field, index) => {
                      ...
                      return (
                        <div key={field.id} className="relative rounded-lg border border-border/80 bg-background p-3 space-y-2">
                          <button
                            type="button"
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                            <div>
                              <label className="text-[11px] font-medium text-muted-foreground block">Key (e.g. ram, size)</label>
                              <Input {...regCreate(`attributes.${index}.key`)} placeholder="ram" className="h-8 text-xs" />
                            </div>
                            <div>
                              <label className="text-[11px] font-medium text-muted-foreground block">Label (e.g. RAM)</label>
                              <Input {...regCreate(`attributes.${index}.label`)} placeholder="RAM" className="h-8 text-xs" />
                            </div>
                            <div>
                              <label className="text-[11px] font-medium text-muted-foreground block">Type</label>
                              <select
                              ...
                              >
                                ...
                                <option value="multi-select">Multi-Select Pills</option>
                          ...
                          {needsOptions && (
                            <div>
                              <label className="text-[11px] font-medium text-muted-foreground block">Options (comma separated)</label>
                              <Input {...regCreate(`attributes.${index}.options`)} placeholder="4GB, 8GB, 16GB, 32GB" className="h-8 text-xs" />
                            </div>
                          )}
                          <div className="flex items-center gap-4 pt-1">
                            <div>
                              <label className="text-[11px] font-medium text-muted-foreground block">Unit (optional)</label>
                              <Input {...regCreate(`attributes.${index}.unit`)} placeholder="e.g. GB, cm, kg" className="h-7 text-xs w-24" />
                            </div>
                            <label className="flex items-center gap-1.5 text-xs text-foreground cursor-pointer mt-4">
                              <input type="checkbox" {...regCreate(`attributes.${index}.required`)} className="rounded border-border" />
                      ...
                      <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            onClick={() => updateAttrAppend({ key: "", label: "", type: "text", options: "", required: false, unit: "" })}
                          >
                            <Plus className="size-3 mr-1" /> Add Attribute
                          </Button>
                        </div>
                            ...
                            return (
                              <div key={field.id} className="relative rounded-lg border border-border/80 bg-background p-3 space-y-2">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                  <div>
                                    <label className="text-[11px] font-medium text-muted-foreground block">Key (e.g. ram, size)</label>
                                    <Input {...regUpdate(`attributes.${index}.key`)} placeholder="ram" className="h-8 text-xs" />
                                  </div>
                                  <div>
                                    <label className="text-[11px] font-medium text-muted-foreground block">Label (e.g. RAM)</label>
                                    <Input {...regUpdate(`attributes.${index}.label`)} placeholder="RAM" className="h-8 text-xs" />
                                  </div>
                                  <div>
                                    <label className="text-[11px] font-medium text-muted-foreground block">Type</label>
                                    <select
                                ...
                                {needsOptions && (
                                  <div>
                                    <label className="text-[11px] font-medium text-muted-foreground block">Options (comma separated)</label>
                                    <Input {...regUpdate(`attributes.${index}.options`)} placeholder="4GB, 8GB, 16GB, 32GB" className="h-8 text-xs" />
                                  </div>
                                )}
                                <div className="flex items-center gap-4 pt-1">
                                  <div>
                                    <label className="text-[11px] font-medium text-muted-foreground block">Unit (optional)</label>
                                    <Input {...regUpdate(`attributes.${index}.unit`)} placeholder="e.g. GB, cm, kg" className="h-7 text-xs w-24" />
                                  </div>
                                  <label className="flex items-center gap-1.5 text-xs text-foreground cursor-pointer mt-4">
                                    <input type="checkbox" {...regUpdate(`attributes.${index}.required`)} className="rounded border-border" />
                                    <span>Required Field</span>
                                  </label>
                                </div>
                              </div>
